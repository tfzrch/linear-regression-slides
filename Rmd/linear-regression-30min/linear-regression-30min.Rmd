...
```{r demo-complete-metrics, echo=FALSE}
demo_complete_metrics <- yardstick::metric_set(
  rmse = yardstick::rmse,
  r2 = yardstick::rsq
)
```

```{r demo-complete, echo=FALSE}
#| cache = TRUE,
#| dependson = "cookie-calories-lm"
#| 
# Generate data for complete LR demo animation

# Tibble of ids for each version, plus random coefs
demo_complete <- tibble(
  id = 1:5,
  m = rnorm(5, mean = lm_cookies$coefficients[2], sd = .35),
  c = rnorm(5, mean = lm_cookies$coefficients[1], sd = .35)
)

# Generate predictions and stats
demo_complete <- demo_complete %>% 
  mutate(
    # Add linear model with the randomly generated coefs
    lm = map2(m, c, function(m, c, lm = lm_cookies) {
      lm$coefficients[2] <- m
      lm$coefficients[1] <- c
      lm
    }),
    # Make predictions using new models
    data = map(lm, function(model, data = random_cookies) {
      data$y_pred <- predict(model, newdata = data)
      data <- data %>% 
        mutate(resid = y - y_pred)
    }),
    # Get RMSE and R^2 for each model
    eval = map(data, demo_complete_metrics, truth = y, estimate = y_pred) %>% 
      map(select, -.estimator) %>% 
      map(pivot_wider, names_from = .metric, values_from = .estimate),
    # Get min/max values for regression line
    line = map(lm, get_lm_line, data = random_cookies)
  ) %>% 
  # Unnest
  unnest_wider(eval) %>% 
  # Sort by RMSE and re-assign ids
  arrange(desc(rmse)) %>% 
  mutate(id = 1:5)

# Split into separate frames for labels and data
demo_complete <- list(
  labels = demo_complete %>% select(id, m, c, rmse, rsq),
  line = demo_complete %>% select(id, line) %>% unnest(line),
  resid = demo_complete %>% select(id, data) %>% unnest(data)
)
```

```{r plt-demo-complete, echo=FALSE}
#| cache = F,
#| dependson = "demo-complete"

# Plot full demo
plt_demo_complete <- ggplot(random_cookies, aes(x, y)) +
  geom_point() +
  labs(
    x = "Ingredients", 
    y = "Calories"
  ...
  geom_line(
    data = demo_complete$line, 
    mapping = aes(group = id), 
    col = pal$vermilion
  ) +
  geom_linerange(
    data = demo_complete$resid, 
    mapping = aes(group = id, ymin = y, ymax = y_pred),
    col = pal$queen.blue
  ) +
  theme(
    axis.text = element_blank()
...
```{r show-plt-demo-complete, echo=FALSE, fig.height=10, fig.width=10}
# Animation
gganimate::animate(plt_demo_complete, renderer = gganimate::magick_renderer())
```
